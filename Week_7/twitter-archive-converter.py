# twitter-archive-converter.py
# 
# convert the tweet archive file, each tweet will be in one line
#
# usage: twitter-archive-converter.py -e 0 /path/to/tweet.js
# the output is saved as json/tweet.json
...
# arguments
options = argparse.ArgumentParser(description="twitter archive convert")

options.add_argument (
    '-e',
    default = 0,
    help = 'convert the entities fields, 1 will convert, 0 will not convert (default: 0)'
)
...
if args.o == "csv":
    try:
        ...
        pass
    parsed_file = base_path + '/csv/' + os.path.basename(str(src_file)).split('.')[0] + '.csv'
else:
    try:
        os.makedirs(base_path + "/json")
    except FileExistsError:
        pass
    parsed_file = base_path + '/json/' + os.path.basename(str(src_file)).split('.')[0] + '.json'
...
with open(parsed_file,'a') as dst_file:
    for tweet in json_data:
        # if -e 0 or not present, do not save the entities and extended_entities fields
        if int(args.e) == 0:
